...
void Map::render(ShaderProgram* program)
{
    ...
    //model_matrix = glm::translate(model_matrix, glm::vec3(-4.5f, 0.0f, 0.0f));
...
bool Map::is_solid(glm::vec3 position, float* penetration_x, float* penetration_y)
{
    ...
    *penetration_y = 0;
    ...
    // If the tile index is 0 i.e. an open space, it is not solid - tile is a map object
    int tile = m_level_data[tile_y * m_width + tile_x];
    if (std::find(nonSolidTiles, nonSolidTiles + numNonSolidTiles, tile) != nonSolidTiles + numNonSolidTiles) return false;
